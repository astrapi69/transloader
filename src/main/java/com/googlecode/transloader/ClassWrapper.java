...
public final class ClassWrapper
{
	...
	/**
	 * Constructs a new <code>ClassWrapper</code> around the given <code>Class</code>. Note that
	 * using an implementation of {@link Transloader} is the recommended way to produce these.
	 *
	 * @param classToWrap
	 *            the <code>Class</code> to wrap
	 */
	...
	/**
	 * Indicates whether or not the wrapped <code>Class</code> is assignable to a <code>Class</code>
	 * with the given name. It takes a parameter of type <code>String</code> instead of
	 * <code>Class</code> so that the test can be performed for <code>Class</code>es that do not
	 * have an equivalent in the caller's <code>ClassLoader</code>.
	 *
	 * @param typeName
	 *            the name of the type to check against
	 * @return true if the wrapped <code>Class</code> is assignable to a <code>Class</code> with the
	 *         given name
	 */
	 ...
	 *
	 * @param classLoader
	 *            the <code>ClassLoader</code> with which to load it
	 * @param className
	 *            the name of the <code>Class</code>
	 * @return the <code>Class</code> with the given name loaded from the given
	...
	public static Class getClassFrom(ClassLoader classLoader, String className)
	{
		...
		catch (ClassNotFoundException e)
		{
			...
			throw new TransloaderException(
				"Unable to load Class '" + className + "' from ClassLoader '" + classLoader + "'.",
				e);
		}
	}
	 ...
	 *
	 * @param classLoader
	 *            the <code>ClassLoader</code> with which to load them
	 * @param classNames
	 *            the names of the <code>Class</code>es
	 * @return the <code>Class</code>es with the given names loaded from the given
	 *         <code>ClassLoader</code>
	 * @throws com.googlecode.transloader.except.TransloaderException
	...
	/**
	 * Retrieves the <code>ClassLoader</code> of the <code>Class</code> of the given
	 * <code>object</code>.
	 *
	 * @param object
	 *            the <code>Object</code> from which to retrieve the <code>ClassLoader</code> (can
	 *            be <code>null</code>)
	 * @return the <code>ClassLoader</code> of the <code>Class</code> of the given
	 *         <code>object</code> if that <code>object</code> is not <code>null</code> and that
	 *         <code>ClassLoader</code> is not <code>null</code>, otherwise
	 *         {@link com.googlecode.transloader.load.BootClassLoader#INSTANCE}
	 */
