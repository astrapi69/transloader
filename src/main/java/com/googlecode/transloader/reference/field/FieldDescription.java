...
public final class FieldDescription implements ReferenceDescription
{
	 ...
	 *
	 * @param field
	 *            the field from which this description is derived
	 * @param setter
	 *            the strategy by which the value of the field is set
	 */
	public FieldDescription(Field field, FieldSetter setter)
	{
		this(((Field)Assert.isNotNull(field)).getDeclaringClass().getName(), field.getName(),
			field.getType().isPrimitive(), Modifier.isTransient(field.getModifiers()), setter);
	}
	 ...
	 *            the name of the <code>Class</code> that declares the field
	 * @param fieldName
	 *            the name of the field
	 * @param isPrimitive
	 *            indicator of whether the reference is to a primitive rather than an {@link Object}
	 * @param isTransient
	 *            indicator of whether the reference is marked transient, excluded from
	 *            serialization
	 * @param setter
	 *            the strategy by which the value of the field is set
	 */
	public FieldDescription(String declaringClassName, String fieldName, boolean isPrimitive,
		boolean isTransient, FieldSetter setter)
	{
	...
	/**
	 * Retrieves the value assigned to the field matching <code>this</code> description on the given
	 * object.
	 *
	 * @param referer
	 ...
	 *             if the <code>Field</code> named by <code>this</code> description does not exist
	 *             on the <code>Class</code> named by <code>this</code> description or the
	 *             <code>Class</code> named by <code>this</code> description is not in the
	 *             <code>referer</code>'s <code>Class</code> hierarchy
	 * @throws TransloaderException
	 *             if the field cannot be located on the <code>referer</code> or the configured
	...
	public Object getValueFrom(Object referer) throws NoSuchFieldException
	{
		...
		catch (IllegalAccessException e)
		{
			throw new TransloaderException(
				"Unable to get value for '" + fieldName + "' from '" + referer + "'.", e);
		}
	}
	 ...
	 * @throws TransloaderException
	 *             if the installed Security Manager does not allow access to the field
	 */
	public void setValueIn(Object referer, Object value)
	{
		...
		{
			throw new TransloaderException("Unable to set value for '" + fieldName + "' to '"
				+ value + "' on '" + referer + "'.", e);
		}
	}
	...
	private void checkClassWasFoundFrom(Class refererClass, Class declaringClass)
		throws NoSuchFieldException
	{
		if (declaringClass == null)
			throw new NoSuchFieldException("No Class named '" + declaringClassName
				+ "' in the hierarchy of '" + refererClass.getName() + "' " + "in ClassLoader '"
				+ refererClass.getClassLoader() + "'.");
	}
	...
	private Class findMatchingClassFrom(Class subclass)
	{
		if (subclass == Object.class)
			return null;
		return declaringClassName.equals(subclass.getName())
			? subclass
			: findMatchingClassFrom(subclass.getSuperclass());
	}
}
