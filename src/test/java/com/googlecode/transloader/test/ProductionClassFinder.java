...
public class ProductionClassFinder
{
	...
	private static File getRootDirectoryOf(Class clazz) throws MalformedURLException
	{
		String classResourceName = DIRECTORY_SEPERATOR
			+ clazz.getName().replace(PACKAGE_SEPERATOR, DIRECTORY_SEPERATOR) + CLASS_EXTENSION;
	...
	{
		return StringUtils.removeEnd(getRelativePath(rootDir, classFile), CLASS_EXTENSION)
			.replace(DIRECTORY_SEPERATOR, PACKAGE_SEPERATOR);
	}
